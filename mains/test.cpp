...
void assign_param(uint32_t ctr, uint32_t depth, test_options *tops) {
    switch (depth) {
    case 0:
        tops->prot = test_prots[ctr];
        break;
    case 1:
        tops->ftype = test_ftype[ctr];
        break;
    case 2:
        tops->sflavor = test_sflavor[ctr];
        break;
    case 3:
        tops->rflavor = test_rflavor[ctr];
        break;
    case 4:
        tops->numots = test_numots[ctr];
        break;
    case 5:
        tops->bitlen = test_bitlen[ctr];
        break;
    case 6:
        tops->nthreads = test_nthreads[ctr];
        break;
    case 7:
        tops->usemecr = test_usemecr[ctr];
        break;
    default:
        cerr << "Test case not recognized, abort" << endl;
        exit(0);
    }
}
...
void run_test_sender(uint32_t numots, uint32_t bitlength, snd_ot_flavor stype,
    ...
    sender->send(numots, bitlength, &X1, &X2, stype, rtype, numthreads,
                 m_fMaskFct);
...
void run_test_receiver(uint32_t numots, uint32_t bitlength, snd_ot_flavor stype,
    ...
    */
    ...
    receiver->receive(numots, bitlength, &choices, &response, stype, rtype,
                      numthreads, m_fMaskFct);
    delete m_fMaskFct;
