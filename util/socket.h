...
class CSocket {
  ...
  public:
    BOOL Socket() {
        ...
        success = (m_hSock = socket(PF_INET, SOCK_STREAM, IPPROTO_TCP)) !=
                  INVALID_SOCKET;
  ...
  public:
    string GetIP() {
        ...
        if (getsockname(m_hSock, (sockaddr *)&addr, (socklen_t *)&addr_len) <
            0) {
            return "";
        }
    ...
    BOOL Connect(string ip, USHORT port, LONG lTOSMilisec = -1) {
        ...
        if (ret >= 0 && lTOSMilisec > 0)
            setsockopt(m_hSock, SOL_SOCKET, SO_RCVTIMEO, (char *)&dw,
                       sizeof(dw));
