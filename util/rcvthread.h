...
// A receive task listens to a particular id and writes incoming data on that id
// into rcv_buf and triggers event
struct rcv_task {
...
class RcvThread : public CThread {
  public:
    ...
    queue<uint8_t *> *add_listener(uint8_t channelid, CEvent *rcv_event,
                                   CEvent *fin_event) {
        rcvlock->Lock();
#ifdef DEBUG_RECEIVE_THREAD
        cout << "Registering listener on channel " << (uint32_t)channelid
             << endl;
#endif

        if (listeners[channelid].inuse || channelid == ADMIN_CHANNEL) {
            cerr << "A listener has already been registered on channel "
                 << (uint32_t)channelid << endl;
        ...
        // cout << "Successfully registered on channel " << (uint32_t) channelid
        // << endl;
    ...
    void ThreadMain() {
        ...
        while (true) {
            ...
            if (rcv_len > 0) {
#ifdef DEBUG_RECEIVE_THREAD
                cout << "Received value on channel " << (uint32_t)channelid
                     << " with " << rcvbytelen << " bytes length (" << rcv_len
                     << ")" << endl;
#endif
                ...
                if (channelid == ADMIN_CHANNEL) {
                    ...
                    // TODO: Right now finish, can be used for other maintenance
                    // tasks
                    free(tmprcvbuf);
