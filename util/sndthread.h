...
class SndThread : public CThread {
  public:
    ...
    void add_snd_task_start_len(uint8_t channelid, uint64_t sndbytes,
                                uint8_t *sndbuf, uint64_t startid,
                                uint64_t len) {
        ...
        // cout << "Adding a new task that is supposed to send " <<
        // task->bytelen << " bytes on channel " << (uint32_t) channelid  <<
        // endl;
...
#ifdef DEBUG_SEND_THREAD
                cout << "Sending on channel " << (uint32_t)channelid
                     << " a message of " << task->bytelen << " bytes length"
                     << endl;
#endif
