...
class CBitVector {
  public:
    ...
    void Create(uint64_t numelementsDimA, uint64_t numelementsDimB,
...
#define GetMask(len) (((1 << (len)) - 1))
    ...
    void SetBitNoMask(int idx, BYTE b) {
        m_pBits[idx >> 3] = (m_pBits[idx >> 3] & C_BIT[idx & 0x7]) |
                            SET_BIT_C[!(b & 0x01)][idx & 0x7];
    }
    ...
    template <class T> void Set2D(T val, int i, int j) {
        Set<T>(val, (i * m_nNumElementsDimB + j) * m_nElementLength,
               m_nElementLength);
    }
