...
void CBitVector::Create(uint64_t bits) {
    ...
    // TODO: check if padding to aes bits is still necessary, otherwise pad to
    // bytes
...
void CBitVector::Create(uint64_t numelementsDimA, uint64_t numelementsDimB,
...
void CBitVector::Create(uint64_t numelementsDimA, uint64_t numelementsDimB,
                        uint64_t elementlength, crypto *crypt) {
...
void CBitVector::SetBits(BYTE *p, uint64_t pos, uint64_t len) {
    ...
    if (remlen) {
        ...
        if (remlen <= uppermask) {
            ...
            m_pBits[posctr] =
                (m_pBits[posctr] & RESET_BIT_POSITIONS[lowermask]) |
                ((temp << lowermask) & 0xFF);
            m_pBits[posctr + 1] =
...
void CBitVector::GetBits(BYTE *p, int pos, int len) {
    ...
    if (remlen) {
        if (remlen <= uppermask) {
            ...
            p[i] = ((m_pBits[posctr] & GET_BIT_POSITIONS[lowermask]) >>
                    lowermask) &
                   0xFF;
...
unsigned int CBitVector::GetInt(int bitPos, int bitLen) {
    ...
    j = 8 - j;
    for (k = bitLen - j; i < (bitPos + bitLen + 7) / 8 - 1;
         i++, j += 8, k -= 8) {
        ret |= m_pBits[i] << j;
    }
...
void CBitVector::EklundhBitTranspose(int rows, int columns) {
    ...
    for (int i = 0, j; i < LOG2_REGISTER_SIZE; i++, srcidx *= 2) {
        ...
        if (i > 2) {
            for (int j = 0; j < rounds; j++) {
                for (lim = rowbptr + destidx; rowbptr < lim;
                     rowaptr++, rowbptr++) {
                    temp_row = *rowaptr;
                    *rowaptr =
                        ((*rowaptr & mask) ^ ((*rowbptr & mask) << srcidx));
                    *rowbptr = ((*rowbptr & invmask) ^
                                ((temp_row & invmask) >> srcidx));
                }
                rowaptr += destidx;
                rowbptr += destidx;
            }
            ...
            for (int j = 0; j < rounds; j++) {
                for (lim = rowbptr + destidx; rowbptr < lim;
                     rowaptr++, rowbptr++) {
                    temp_row = *rowaptr;
                    *rowaptr = ((*rowaptr & invmask) ^
                                ((*rowbptr & invmask) >> srcidx));
                    *rowbptr =
                        ((*rowbptr & mask) ^ ((temp_row & mask) << srcidx));
                }
                rowaptr += destidx;
                rowbptr += destidx;
            }
    ...
    for (int i = LOG2_REGISTER_SIZE, j, swapoffset = 1, dswapoffset;
         i < numiters; i++, srcidx *= 2, swapoffset = swapoffset << 1) {
        destidx = offset * srcidx;
    ...
    if (columns > rows) {
        ...
        for (int i = 0; i < columns / rows; i++) {
            ...
            for (int j = 0; j < rows;
                 j++, rowaptr += rowregsize, rowbptr += offset) {
