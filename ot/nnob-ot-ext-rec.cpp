...
BOOL NNOBOTExtRec::receiver_routine(uint32_t id, uint64_t myNumOTs) {
    ...
    double totalMtxTime = 0, totalTnsTime = 0, totalHshTime = 0,
           ...
           totalMaskTime = 0, totalEnqueueTime = 0;
...
nnob_rcv_check_t NNOBOTExtRec::EnqueueSeed(uint8_t *T0, uint64_t otid,
                                           uint64_t numblocks) {
...
void NNOBOTExtRec::ComputeOWF(queue<nnob_rcv_check_t> *check_buf_q,
    ...
    linking_t *perm;
    uint8_t *rcv_buf_perm = check_chan->blocking_receive_id_len(
        (uint8_t **)&perm, &tmpid, &tmpnblocks);
    ...
    uint64_t checkbytelen =
        min(bufrowbytelen, bits_in_bytes(m_nOTs - check_buf.otid));
    ...
    uint8_t *receiver_choicebits =
        m_vChoices->GetArr() + ceil_divide(check_buf.otid, 8);
    CBitVector tmp;
    ...
    for (i = 0; i < m_nChecks; i++, sender_permchoicebitptr++) {
        ...
        cout << (dec) << i << "-th check: between " << perm[i].ida << ", and "
             << perm[i].idb << endl;
#endif
        for (j = 0; j < receiver_hashes; j++, outptr += OWF_BYTES) {
            ...
            assert((*sender_permchoicebitptr) == 0 ||
                   (*sender_permchoicebitptr == 1));
...
#else
            // m_cCrypt->hash_buf(outptr, OWF_BYTES, tmpbuf, checkbytelen,
            // hash_buf);
