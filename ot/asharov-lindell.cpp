...
BOOL AsharovLindell::ReceiverIFC(int nSndVals, int nOTs, CBitVector& choices,
CSocket& socket, BYTE* ret)
{
        int nBufSize = nSndVals * m_NPState.field_size * nOTs;
        BYTE* pBuf = new BYTE[nBufSize]; //stores the answer bits of the
receiver (d) in the Naor Pinkas Protocol
        ...
        for (int i = 0; i < nOTs; i++) {
                //sample random hi -- sample random element x in Zp, and then
compute x^{(p-1)/q} mod p
                do
                {
                        mpz_urandomb(ztmp, m_NPState.rnd_state,
m_NPState.field_size * 8);
                        mpz_mod(ztmp2, ztmp, m_NPState.p);
                        mpz_powm(h[i], ztmp2, div, m_NPState.p);
                } while(!(mpz_cmp_ui(h[i], (unsigned long int) 1) )  );
                //cout << h[i] << endl;
                //mpz_urandomb(ztmp, m_NPState.rnd_state, m_NPState.field_size *
8);
                //mpz_mod(ztmp2, ztmp, m_NPState.p);
...
8);
        ...
        // For every i, compute ki = u^alphai and then xi^\sigma = vi^\sigma XOR
KDF(ki^\sigma)
        ////////////////////////////////////////////////////////////////////////////
...
BOOL AsharovLindell::SenderIFC(int nSndVals, int nOTs, CSocket& socket, BYTE*
ret)
{
        ...
        {
                //mpz_init(k1[k]);
                mpz_import(pH, m_NPState.field_size, 1, sizeof(pBufIdx[0]), 0,
0, pBufIdx);
...
void AsharovLindell::Receiver(uint32_t nSndVals, uint32_t nOTs,
    ...
    BYTE* pBuf = new BYTE[nBufSize]; //stores the answer bits of the receiver
(d) in the Naor Pinkas Protocol
...
compute x^2 mod p
    ...
    for (int i = 0, idx = 0; i < nOTs; i++) {
            if (!choices.GetBit(i))
            {
                    Miracl_mulbrick(&bg, beta[i].getbig(), xtmp.getbig(),
ytmp.getbig());
                    Miracl_InitPoint(&h0, xtmp, ytmp);
                    //ecurve_mult(beta[i].getbig(), g.get_point(),
h0.get_point());
                    h1 = h[i];
            }
            else
...
KDF(ki^\sigma)
    ...
    for (int k = 0; k < nOTs; k++)
    {
            ...
            Miracl_mulbrick(&bu, beta[k].getbig(), xtmp.getbig(),
ytmp.getbig());
...
void AsharovLindell::Sender(uint32_t nSndVals, uint32_t nOTs, channel *chan,
                            BYTE *ret) {
    /*	//buffer for sending u
            int coordSize = (m_SecParam+7)/8;//(state.field_size/8) + 4;
    ...
    #else
                    ecurve2_mult(alpha.getbig(), pH.get_point(),
    pK.get_point());
    #endif
