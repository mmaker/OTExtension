...
class OTExt {
  ...
  protected:
    void Init(uint32_t nSndVals, crypto *crypt, RcvThread *rcvthread,
              SndThread *sndthread, uint32_t nbaseOTs, uint32_t nbasekeys) {
        m_nSndVals = nSndVals;
        m_cCrypt = crypt;
    ...
    // The flag whether to use min-entropy correlation robustness instead of
...
static void fillRndMatrix(uint8_t *seed, uint64_t **mat, uint64_t cols,
                          uint64_t rows, crypto *crypt) {
    uint32_t columnlen = ceil_divide(cols, sizeof(uint64_t) * 8);
    prf_state_ctx tmpstate;
    crypt->init_prf_state(&tmpstate, seed);
    for (uint32_t i = 0; i < rows; i++) {
        gen_rnd_bytes(&tmpstate, (uint8_t *)mat[i],
                      columnlen * sizeof(uint64_t));
    }
...
static void BitMatrixMultiplication(uint8_t *resbuf, uint64_t resbytelen,
                                    uint8_t *invec, uint32_t inveclen,
    ...
    for (uint32_t i = 0, j; i < inveclen; i++) {
        ...
        for (j = 0; j < columniters; j++) {
            ...
            // cout << "rowbit = " << rowbit << ", matrix[i][j] = " <<
            // matrix[i][j] << ", tmpbuf = " << tmpbuf[j] << endl;
        }
    }
...
#ifdef FIXED_KEY_AES_HASHING
static const uint8_t fixed_key_aes_seed[32] = {
...
#ifdef FIXED_KEY_AES_HASHING
inline void FixedKeyHashing(AES_KEY_CTX *aeskey, BYTE *outbuf, BYTE *inbuf,
                            BYTE *tmpbuf, uint64_t id, uint32_t bytessecparam,
                            crypto *crypt) {
