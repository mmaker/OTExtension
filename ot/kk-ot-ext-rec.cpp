...
// The send buffer
    ...
    double totalMtxTime = 0, totalTnsTime = 0, totalHshTime = 0,
           totalRcvTime = 0, totalSndTime = 0, totalChkTime = 0,
           totalMaskTime = 0, totalChoiceTime = 0;
...
// nSize = bits_in_bytes(m_nBaseOTs * OTsPerIteration);
        ...
        gettimeofday(&tempStart, NULL);
#endif
        // SetOutput(maskbuf, otid, OTsPerIteration, &mask_queue,
        // chan);//ReceiveAndUnMask(chan);
...
#ifdef OTTiming
    cout << "Receiver time benchmark for performing " << myNumOTs << " OTs on "
...
void KKOTExtRec::GenerateChoiceCodes(CBitVector &choicecodes, CBitVector &vSnd,
                                     uint32_t startpos, uint32_t len) {
    uint32_t tmpchoice;
    uint32_t otid = startpos;
    uint32_t ncolumnsbyte =
        ceil_divide(len, m_nCodeWordBits) * m_nCodeWordBytes;
    uint32_t choicecodebitlen = ceil_log2(m_nSndVals);
    ...
    for (uint32_t pos = 0; pos < len; pos += m_nCodeWordBits) {
        ...
        for (uint32_t j = 0; j < min(len - pos, m_nCodeWordBits); j++, otid++) {
            tmpchoice = m_vChoices->Get<uint32_t>(otid * choicecodebitlen,
                                                  choicecodebitlen);
            // cout << "otid = " << otid << ", choice = " << tmpchoice << endl;
            choicecodes.SetBytes((uint8_t *)m_vCodeWords[tmpchoice],
                                 j * m_nCodeWordBytes, m_nCodeWordBytes);
        }
        // cout << "Using codeword " << (hex) << m_vCodeWords[tmpchoice][0] <<
        // m_vCodeWords[tmpchoice][1] << (hex) <<
        //		m_vCodeWords[tmpchoice][2] << m_vCodeWords[tmpchoice][3]
        //<< (dec) << endl;
        ...
        // XOR these transposed choice bits blockwise on the matrix that is sent
        // to S
        for (uint32_t j = 0; j < m_nCodeWordBits; j++) {
            vSnd.XORBytes(choicecodes.GetArr() + j * m_nCodeWordBytes,
                          (pos >> 3) + j * ncolumnsbyte, m_nCodeWordBytes);
        }
    }
    // cout << "vSnd Out: ";
    // vSnd.PrintHex(0, ncolumnsbyte*len);
...
void KKOTExtRec::KKHashValues(CBitVector &T, CBitVector &seedbuf,
                              CBitVector *maskbuf, uint64_t OT_ptr,
    ...
    if (m_eSndOTFlav != Snd_GC_OT) {
        for (uint64_t i = 0; i < OT_len; i++, Tptr += m_nBlockSizeBytes,
            ...
            cout << (dec) << " ("
                 << m_vChoices.Get<uint32_t>(global_OT_ptr * choicecodebits,
                                             choicecodebits) << ")" << endl;
#endif
        }
        // TODO: difference is in here!! (could be solved by giving the
        // bit-length as parameter in the function call)
        // m_fMaskFct->expandMask(m_vTempOTMasks, seedbuf.GetArr(), OT_ptr,
        // OT_len, m_nBitLength, m_cCrypt);
        m_fMaskFct->expandMask(maskbuf, seedbuf.GetArr(), 0, OT_len,
                               m_nBitLength * choicecodebits, m_cCrypt);
...
void KKOTExtRec::KKSetOutput(CBitVector *maskbuf, uint64_t otid, uint64_t otlen,
...
void KKOTExtRec::KKReceiveAndUnMask(channel *chan,
                                    queue<mask_block *> *mask_queue) {
    ...
    tmpmaskbuf =
        (uint8_t *)malloc(bits_in_bytes(choicecodebits * m_nBitLength));
    ...
    while (chan->data_available() && !(mask_queue->empty())) {
        ...
        m_vRet->Copy(*tmpblock->buf,
                     bits_in_bytes(choicecodebits * startotid * m_nBitLength),
                     valsize);
        ...
        for (uint32_t i = 0; i < otlen; i++) {
            tmpchoice = m_vChoices->Get<uint32_t>(
                (startotid + i) * choicecodebits, choicecodebits);
#ifdef DEBUG_KK_OTBREAKDOWN
            cout << "choice in " << i << "-th 1-out-of-" << m_nSndVals
                 << " OT: " << tmpchoice << endl;
#endif
            if (tmpchoice != 0 && tmpchoice != m_nSndVals - 1) {
                // tmpmask = vRcv.Get<uint32_t>((tmpchoice-1) * valsize * 8 + i
                // * choicecodebits*m_nBitLength, choicecodebits*m_nBitLength);
                vRcv.GetBits(tmpmaskbuf, (tmpchoice - 1) * valsize * 8 +
                                             i * choicecodebits * m_nBitLength,
                             choicecodebits * m_nBitLength);
#ifdef DEBUG_KK_OTBREAKDOWN
                cout << "Accessing bit-address "
                     << (tmpchoice - 1) * valsize * 8 +
                            i * choicecodebits * m_nBitLength
                     << " with bit-length " << choicecodebits * m_nBitLength
                     << endl;
#endif
            } else {
                memset(tmpmaskbuf, 0,
                       bits_in_bytes(choicecodebits * m_nBitLength));
            }

#ifdef DEBUG_KK_OTBREAKDOWN
            cout << "Mask " << tmpchoice << ": " << (hex);
            for (uint32_t j = 0;
                 j < bits_in_bytes(choicecodebits * m_nBitLength); j++)
                cout << (uint32_t)tmpmaskbuf[j];
            cout << (dec) << endl;
            // tmpmask ^= tmpblock.Get<uint32_t>(i * choicecodebits,
            // choicecodebits);
            // m_vRet.XOR(tmpmask, (startotid + i), choicecodebits);
            cout << "startotid = " << startotid << ", start = "
                 << (startotid + i) * choicecodebits * m_nBitLength
                 << ", len = " << choicecodebits * m_nBitLength << endl;
#endif
            // m_vRet.XORBytes(tmpmaskbuf, bits_in_bytes((startotid + i) *
            // choicecodebits * m_nBitLength), bits_in_bytes(choicecodebits *
            // m_nBitLength));
            m_vRet->XORBits(tmpmaskbuf,
                            (startotid + i) * choicecodebits * m_nBitLength,
                            choicecodebits * m_nBitLength);
        }
#ifdef DEBUG_KK_OTBREAKDOWN
...
void KKOTExtRec::KKMaskBaseOTs(CBitVector &T, CBitVector &SndBuf,
                               uint64_t numblocks) {
