...
BOOL OTExtSnd::send(uint32_t numOTs, uint32_t bitlength, CBitVector *x0,
                    CBitVector *x1, snd_ot_flavor stype, rec_ot_flavor rtype,
...
BOOL OTExtSnd::start_send(uint32_t numThreads) {
    ...
    uint32_t wd_size_bits =
        m_nBlockSizeBits; // pad_to_power_of_two(m_nBaseOTs);//1 <<
                          // (ceil_log2(m_nBaseOTs));
    // uint64_t numOTs = ceil_divide(PadToMultiple(m_nOTs, wd_size_bits),
    // numThreads);
...
void OTExtSnd::BuildQMatrix(CBitVector &T, uint64_t OT_ptr,
                            uint64_t numblocks) {
    BYTE *Tptr = T.GetArr();
    ...
    uint64_t wd_size_bytes =
        m_nBlockSizeBytes; // pad_to_power_of_two(m_nBaseOTs/8);//1 <<
    ...
    for (uint64_t k = 0, b; k < m_nBaseOTs; k++) {
        *counter = global_OT_ptr;
        for (b = 0; b < iters; b++, (*counter)++, Tptr += AES_BYTES) {
            ...
            cout << "k = " << k << ": " << (hex) << ((uint64_t *)Tptr)[0]
                 << ((uint64_t *)Tptr)[1] << (hex) << endl;
#endif
        }
...
// XOR m_nU on top
void OTExtSnd::UnMaskBaseOTs(CBitVector &T, CBitVector &RcvBuf,
                             uint64_t numblocks) {
...
void OTExtSnd::ReceiveMasks(CBitVector &vRcv, channel *chan,
    ...
    rcvbufptr =
        chan->blocking_receive_id_len(&rcvbuftmpptr, &tmpctr, &tmpotlen);
...
#ifdef GENERATE_T_EXPLICITELY
    if (m_eRecOTFlav == Rec_R_OT) {
        vRcv.SetBytes(rcvbuftmpptr, bits_in_bytes(processedOTs),
                      bits_in_bytes((m_nBaseOTs - startpos) *
                                    processedOTs)); // AttachBuf(rcvbuftmpptr,
                                                    // bits_in_bytes(m_nBaseOTs
                                                    // * OTsPerIteration));
        vRcv.SetBytes(rcvbuftmpptr +
                          bits_in_bytes((m_nBaseOTs - startpos) * processedOTs),
                      bits_in_bytes(m_nBaseOTs * processedOTs),
                      bits_in_bytes((m_nBaseOTs - startpos) * processedOTs));
    } else {
        vRcv.SetBytes(
            rcvbuftmpptr, 0,
            2 * bits_in_bytes(m_nBaseOTs *
                              processedOTs)); // AttachBuf(rcvbuftmpptr,
                                              // bits_in_bytes(m_nBaseOTs *
                                              // OTsPerIteration));
    }
#else
    vRcv.SetBytes(rcvbuftmpptr, bits_in_bytes(startpos * processedOTs),
                  bits_in_bytes((m_nBaseOTs - startpos) *
                                processedOTs)); // AttachBuf(rcvbuftmpptr,
                                                // bits_in_bytes(m_nBaseOTs *
                                                // OTsPerIteration));
...
void OTExtSnd::GenerateSendAndXORCorRobVector(CBitVector &Q, uint64_t OT_len,
                                              channel *chan) {
...
void OTExtSnd::HashValues(CBitVector &Q, CBitVector *seedbuf,
                          CBitVector *snd_buf, uint64_t OT_ptr, uint64_t OT_len,
                          uint64_t **mat_mul) {
    ...
    for (uint64_t i = 0; i < OT_len; global_OT_ptr++, i++, Qptr += 2) {
        for (u = 0; u < m_nSndVals; u++) {
            ...
            for (uint32_t p = 0; p < rowbytelen; p++)
                cout << setw(2) << setfill('0')
                     << (uint32_t)(Q.GetArr() + i * wd_size_bytes)[p];
            cout << (dec) << endl;
#endif
            ...
            if (m_eSndOTFlav != Snd_GC_OT) {
                ...
                memcpy(inbuf + sizeof(uint64_t), Q.GetArr() + i * wd_size_bytes,
                       rowbytelen);
...
// m_vValues[1].PrintHex();
        ...
        if (m_eSndOTFlav != Snd_GC_OT) {
            ...
            for (uint32_t u = 0; u < m_nSndVals; u++) {
                m_fMaskFct->expandMask(&(snd_buf[u]), seedbuf[u].GetArr(), 0,
    ...
    void OTExtSnd::MaskAndSend(CBitVector * snd_buf, uint64_t OT_ptr,
    ...
    BOOL OTExtSnd::verifyOT(uint64_t NumOTs) {
        ...
        for (uint64_t i = 0; i < NumOTs; i += OTsPerIteration) {
            processedOTBlocks =
                min((uint64_t)NUMOTBLOCKS, ceil_divide(NumOTs - i, AES_BITS));
            OTsPerIteration = min(processedOTBlocks * AES_BITS, NumOTs - i);
            nSnd = ceil_divide(OTsPerIteration * m_nBitLength, 8);

            chan->send_id_len(m_vValues[0]->GetArr() +
                                  bits_in_bytes(i * m_nBitLength),
                              nSnd, i, OTsPerIteration);
            chan->send_id_len(m_vValues[1]->GetArr() +
                                  bits_in_bytes(i * m_nBitLength),
                              nSnd, i, OTsPerIteration);
            // sndthread->add_snd_task_start_len(0, nSnd, m_vValues[0].GetArr()
            // + bits_in_bytes(i * m_nBitLength), i, OTsPerIteration);
            // sndthread->add_snd_task_start_len(0, nSnd, m_vValues[1].GetArr()
            // + bits_in_bytes(i * m_nBitLength), i, OTsPerIteration);
    ...
    void OTExtSnd::ComputePKBaseOTs() {
        channel *chan = new channel(0, m_cRcvThread, m_cSndThread);
        uint8_t *pBuf =
            (uint8_t *)malloc(m_cCrypt->get_hash_bytes() * m_nBaseOTs);
        uint8_t *keyBuf =
            (uint8_t *)malloc(m_cCrypt->get_aes_key_bytes() * m_nBaseOTs);
