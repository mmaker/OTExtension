...
class XORMasking : public MaskingFunction {
  public:
    ...
    void Mask(uint32_t progress, uint32_t processedOTs, CBitVector **values,
              CBitVector *snd_buf, snd_ot_flavor protocol) {
        uint32_t nsndvals = 2;

        if (protocol == Snd_OT) {
            snd_buf[0].XORBytes(values[0]->GetArr() +
                                    ceil_divide(progress * m_nBitLength, 8),
                                0, ceil_divide(processedOTs * m_nBitLength, 8));
            snd_buf[1].XORBytes(values[1]->GetArr() +
                                    ceil_divide(progress * m_nBitLength, 8),
                                0, ceil_divide(processedOTs * m_nBitLength, 8));
        } else if (protocol == Snd_C_OT) {
            values[0]->SetBytes(
                snd_buf[0].GetArr(), ceil_divide(progress * m_nBitLength, 8),
                ceil_divide(
                    processedOTs * m_nBitLength,
                    8)); //.SetBits(hash_buf, i*m_nBitLength, m_nBitLength);
    ...
    void UnMask(uint32_t progress, uint32_t processedOTs, CBitVector *choices,
        ...
        if (protocol == Snd_OT) {
            if (m_nBitLength & 0x07) {
                ...
                for (uint32_t u, i = progress, l = 0; i < lim;
                     i++, gprogress += m_nBitLength, l += m_nBitLength) {
                    u = (uint32_t)choices->GetBitNoMask(i);
                    output->XORBitsPosOffset(rcv_buf->GetArr(),
                                             (u * offset) + l, gprogress,
                                             m_nBitLength);
                }
            } else {
                offset = processedOTs * bytelen;
                for (uint32_t u, i = progress, l = 0; i < lim;
                     i++, gprogress += bytelen, l += bytelen) {
                    u = (uint32_t)choices->GetBitNoMask(i);
                    output->SetXOR(rcv_buf->GetArr() + (u * offset) + l,
                                   tmpmask->GetArr() + l, gprogress, bytelen);
                }
            }
            ...
            if (m_nBitLength & 0x07) {
                ...
                for (uint32_t u, i = progress, l = 0; i < lim;
                    ...
                    if (choices->GetBitNoMask(i)) {
                        output->XORBitsPosOffset(rcv_buf->GetArr(), l,
                                                 gprogress, m_nBitLength);
                    }
                }
            } else {
                // output.Copy(tmpmask.GetArr() + gprogress, gprogress, bytelen
                // * processedOTs);
                output->Copy(tmpmask->GetArr(), gprogress,
                             bytelen * processedOTs);
                for (uint32_t i = progress, l = 0; i < lim;
                     i++, l += bytelen, gprogress += bytelen) {
                    if (choices->GetBitNoMask(i)) {
                        output->XORBytes(rcv_buf->GetArr() + l, gprogress,
                                         bytelen);
                    }
                }
            }
            ...
            output->Copy(tmpmask->GetArr(), gprogress,
                         bits_in_bytes(processedOTs * m_nBitLength));
        }
    };
    ...
    void expandMask(CBitVector *out, BYTE *sbp, uint32_t offset,
        ...
        if (bitlength <= AES_KEY_BITS) {
            ...
            for (uint32_t i = 0; i < processedOTs;
                 i++, sbp += AES_KEY_BYTES, pos += bitlength) {
