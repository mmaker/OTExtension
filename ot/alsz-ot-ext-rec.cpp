...
BOOL ALSZOTExtRec::receiver_routine(uint32_t id, uint64_t myNumOTs) {
    ...
    // these two values are only required for the min entropy correlation
    ...
    double totalMtxTime = 0, totalTnsTime = 0, totalHshTime = 0,
           ...
           totalMaskTime = 0, totalEnqueueTime = 0, totalOutputSetTime = 0;
...
#endif
        check_buf.push(
            EnqueueSeed(T.GetArr(), vSnd.GetArr(), otid, processedOTBlocks));
#ifdef OTTiming
...
// ot_chan->send_id_len(vSnd.GetArr(), nSize, otid, OTsPerIteration);
        ...
        if (!m_bUseMinEntCorRob) {
            ...
            gettimeofday(&tempStart, NULL);
#endif
            HashValues(&T, &seedbuf, &maskbuf, otid,
                       min(lim - otid, OTsPerIteration), rndmat);
#ifdef OTTiming
            gettimeofday(&tempEnd, NULL);
        ...
        if (check_chan->data_available()) {
            if (m_bUseMinEntCorRob) {
                ...
                Ttmp.Copy(check_tmp.T0, 0,
                          check_tmp.numblocks * m_nBlockSizeBytes);
            }
            ComputeOWF(&check_buf, check_chan);
            if (m_bUseMinEntCorRob) {
                ReceiveAndXORCorRobVector(
                    Ttmp, check_tmp.numblocks * wd_size_bits, mat_chan);
                Ttmp.Transpose(wd_size_bits, OTsPerIteration);
                HashValues(&Ttmp, &seedbuf, &maskbuf, check_tmp.otid,
                           min(lim - check_tmp.otid,
                               check_tmp.numblocks * wd_size_bits),
                           rndmat);
            }
...
alsz_rcv_check_t ALSZOTExtRec::EnqueueSeed(uint8_t *T0, uint8_t *T1,
...
void ALSZOTExtRec::ComputeOWF(queue<alsz_rcv_check_t> *check_buf_q,
                              channel *check_chan) { // linking_t* permbits, int
                                                     // nchecks, int otid, int
                                                     // processedOTs, BYTE*
                                                     // outhashes) {
    ...
    linking_t *perm;
    uint8_t *rcv_buf = check_chan->blocking_receive_id_len((uint8_t **)&perm,
                                                           &tmpid, &tmpnblocks);
    ...
    // the bufsize has to be padded to a multiple of the PRF-size since we will
    // omit boundary checks there
    uint32_t i, k, j;
    uint64_t bufrowbytelen =
        m_nBlockSizeBytes *
        check_buf
            .numblocks; // seedptr->expstrbitlen>>3;//(CEIL_DIVIDE(processedOTs,
                        // wd_size_bits) * wd_size_bits) >>3;
...
// XOR all four possibilities
        ...
        cout << i << "-th check: between " << perm[i].ida << ", and "
             << perm[i].idb << ": " << endl;
#endif

        for (k = 0; k < iters; k++) {
            ((uint64_t *)tmpbuf[0])[k] =
                ((uint64_t *)ka[0])[k] ^ ((uint64_t *)kb[0])[k];
            ((uint64_t *)tmpbuf[1])[k] =
                ((uint64_t *)ka[0])[k] ^ ((uint64_t *)kb[1])[k];
            ((uint64_t *)tmpbuf[2])[k] =
                ((uint64_t *)ka[1])[k] ^ ((uint64_t *)kb[0])[k];
            ((uint64_t *)tmpbuf[3])[k] =
                ((uint64_t *)ka[1])[k] ^ ((uint64_t *)kb[1])[k];
        }
        sha = shatmp;
        ...
        /*		for(j = 0; j < receiver_hashes; j++, outptr+=OWF_BYTES)
        {
                                ...
                                for(k = 0; k < iters; k++) {
                                        ((uint64_t*) tmpbuf)[k] = ((uint64_t*)
        kaptr)[k] ^ ((uint64_t*) kbptr)[k];
                                }
        ...
        (uint32_t) tmpbuf[t];
                                ...
                                gettimeofday(&tstart, NULL);
                #endif
                                sha512_hash(outptr, OWF_BYTES, tmpbuf,
        bufrowbytelen, hash_buf);

                                //m_cCrypt->hash_buf(outptr, OWF_BYTES, tmpbuf,
        bufrowbytelen, hash_buf);
                #ifdef OTTiming_PRECISE
                                gettimeofday(&tend, NULL);
    ...
    check_chan->send_id_len(outhashes, outhashbytelen, check_buf.otid,
                            check_buf.numblocks);
#ifdef OTTiming_PRECISE
