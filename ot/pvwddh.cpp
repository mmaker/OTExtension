...
void PVWDDH::Receiver(uint32_t nSndVals, uint32_t nOTs, CBitVector &choices,
                      channel *chan, uint8_t *retbuf) {
    ...
    // Second step: for each OT generate and send a public-key and receive
    // challenge + send compute proof
    ...
    for (i = 0; i < nOTs; i++) {
        // generate r_i at random and compute g_i = g_sigma_i ^ r_i and h_i =
        // h_sigma_i ^ r_i
    ...
    // Third step: receive the seeds to the KDF from the sender and generate a
    // random string from the chosen one
    ...
    for (i = 0; i < nOTs;
         i++, rcvbufptr += (2 * fe_bytes), retbufptr += hash_bytes) {
        // convert the received bytes to a field element, compute u_i ^ r_i, and
        // convert u_i^r_i back to bytes
...
void PVWDDH::Sender(uint32_t nSndVals, uint32_t nOTs, channel *chan,
                    uint8_t *retbuf) {
    ...
    for (i = 0; i < nOTs; i++) {
        ...
        for (j = 0; j < 2; j++) {
            ...
            bg[j]->pow(gs, s);
            bh[j]->pow(ht, t);
            u = m_cPKCrypto->get_fe(); // TODO: there is sth weird going on
                                       // here, get new fe to avoid this problem
            u->set_mul(gs, ht);

            v = m_cPKCrypto->get_fe(); // TODO: there is sth weird going on
                                       // here, get new fe to avoid this problem
    ...
    // Group check is omitted because both parties use the pre-generated NIST
    // parameters
