...
class KKOTExtRec : public OTExtRec {
     ...
     */
  public:
    KKOTExtRec(uint32_t nSndVals, crypto *crypt, RcvThread *rcvthread,
               SndThread *sndthread) {
        uint32_t numbaseots = max((uint64_t)2 * crypt->get_seclvl().symbits,
                                  pad_to_power_of_two(nSndVals));

        assert(pad_to_power_of_two(nSndVals) ==
               nSndVals); // TODO right now only supports power of two nSndVals
        assert(numbaseots == 256); // TODO: right now only 256 base OTs work due
                                   // to the size of the code
        InitRec(nSndVals, crypt, rcvthread, sndthread,
                2 * crypt->get_seclvl().symbits);
  ...
  private:
    void GenerateChoiceCodes(CBitVector &choicecodes, CBitVector &vSnd,
                             uint32_t startpos, uint32_t len);
    void KKSetOutput(CBitVector *maskbuf, uint64_t otid, uint64_t otlen,
                     queue<mask_block *> *mask_queue, channel *chan);
    void KKHashValues(CBitVector &T, CBitVector &seedbuf, CBitVector *maskbuf,
