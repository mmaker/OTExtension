...
BOOL NaorPinkasNoRO::ReceiverIFC(int nSndVals, int nOTs, CBitVector& choices,
CSocket& socket, BYTE* ret)
{
        ...
        for(int k = 0; k < nOTs; k++)
        {
                //if the choice bit is zero take the first value, else the
second
                mpz_import(w, m_NPState.field_size, 1, sizeof(pBuf[0]), 0, 0,
pBufIdx+(choices.GetBit(k) * m_NPState.field_size));
...
BOOL NaorPinkasNoRO::SenderIFC(int nSndVals, int nOTs, CSocket& socket, BYTE*
ret)
{
        ...
        mpz_t w0, w1, s0[nOTs], s1[nOTs], r0[nOTs], r1[nOTs], R0, R1, x, y, Y,
Z0, Z1, z0, z1, ztmp, w;
        ...
        //Fixed Point Exponentiation of the generator g and precompute all
possible values
        FixedPointExp brg(m_NPState.g, m_NPState.p, m_NPState.field_size*8);
        ...
        {
                //get y, z0, and z1
                mpz_import(y, m_NPState.field_size, 1, sizeof(pBuf[0]), 0, 0,
pBufIdx);
                pBufIdx += m_NPState.field_size;
                mpz_import(z0, m_NPState.field_size, 1, sizeof(pBuf[0]), 0, 0,
pBufIdx);
                pBufIdx += m_NPState.field_size;
                mpz_import(z1, m_NPState.field_size, 1, sizeof(pBuf[0]), 0, 0,
pBufIdx);
...
void NaorPinkasNoRO::Receiver(uint32_t nSndVals, uint32_t nOTs,
                              CBitVector &choices, channel *chan, BYTE *ret) {
    ...
    g = m_cPKCrypto->get_generator();//    Miracl_InitPoint(&g, *m_X, *m_Y);//g
    = EC2(*m_X ,*m_Y);
    ...
    bg->pow(x, a);//Miracl_mulbrick(&bg, a.getbig(), xtmp.getbig(),
    ytmp.getbig());
    //Miracl_InitPoint(&x, xtmp, ytmp);//x = EC2(xtmp, ytmp);
    //epoint2_set(xtmp.getbig(), ytmp.getbig(), 0, x.get_point());//x.get(xtmp,
    ytmp);
    //x = g;
    //x *= a;
    ...
    for(uint32_t k = 0; k < nOTs; k++)
    {
            ...
            bg->pow(y, b[k]);//Miracl_mulbrick(&bg, b[k].getbig(),
    xtmp.getbig(), ytmp.getbig());
            //Miracl_InitPoint(&y, xtmp, ytmp);//y = EC2(xtmp,
    ytmp);//epoint2_set(xtmp.getbig(), ytmp.getbig(), 0, y.get_point());//y =
    ECn(xtmp, ytmp);
            ...
            if(!choices.GetBit(k))
            {
                    bx->pow(z0, b[k]);//Miracl_mulbrick(&bx, b[k].getbig(),
    xtmp.getbig(), ytmp.getbig());
                    //Miracl_InitPoint(&z0, xtmp, ytmp);//z0 = EC2(xtmp,
    ytmp);//epoint2_set(xtmp.getbig(), ytmp.getbig(), 0, z0.get_point());//z0 =
    ECn(xtmp, ytmp);
                    bg->pow(z1, btmp);//Miracl_mulbrick(&bg, btmp.getbig(),
    xtmp.getbig(), ytmp.getbig());
                    //Miracl_InitPoint(&z1, xtmp, ytmp);//z1 = EC2(xtmp,
    ytmp);//epoint2_set(xtmp.getbig(), ytmp.getbig(), 0, z1.get_point());//z1 =
    ECn(xtmp, ytmp);
            }
            else
            {
                    bg->pow(z0, btmp);//Miracl_mulbrick(&bg, btmp.getbig(),
    xtmp.getbig(), ytmp.getbig());
                    //Miracl_InitPoint(&z0, xtmp, ytmp);//z0 = EC2(xtmp,
    ytmp);//epoint2_set(xtmp.getbig(), ytmp.getbig(), 0, z0.get_point());//z0 =
    ECn(xtmp, ytmp);//z0.get(xtmp, ytmp);
                    bx->pow(z1, b[k]);//Miracl_mulbrick(&bx, b[k].getbig(),
    xtmp.getbig(), ytmp.getbig());
                    //Miracl_InitPoint(&z1, xtmp, ytmp);//z1 = EC2(xtmp,
    ytmp);//epoint2_set(xtmp.getbig(), ytmp.getbig(), 0, z1.get_point());//z1 =
    ECn(xtmp, ytmp);
            ...
            //export - first y, then z0, and lastly z1
            y->export_to_bytes(pBufIdx);//PointToByteArray(pBufIdx, coordSize,
    y);
            pBufIdx += febytelen;//(coordSize + 1);
            z0->export_to_bytes(pBufIdx);//PointToByteArray(pBufIdx, coordSize,
    z0);
            pBufIdx += febytelen;//(coordSize + 1);
            z1->export_to_bytes(pBufIdx);//PointToByteArray(pBufIdx, coordSize,
    z1);
            pBufIdx += febytelen;//(coordSize + 1);
            //printepoint(g);
            //printepoint(x);
    ...
    for(uint32_t k = 0; k < nOTs; k++)
    {
            ...
            tmp->import_from_bytes(pBufIdx+(choices.GetBit(k) *
    febytelen));//ByteArrayToPoint(&w, coordSize, pBufIdx+(choices.GetBit(k) *
    (coordSize+1)));
            //w->print();
            //b[k]->print();
            ...
            //w->print();
            //export result and hash
            w->export_to_bytes(cpybuf);//PointToByteArray(pBufIdx, coordSize,
    w);
...
void NaorPinkasNoRO::Sender(uint32_t nSndVals, uint32_t nOTs, channel *chan,
                            BYTE *ret) {
    /*num **s0, **s1, **r0, **r1, *w;//Big s0[nOTs], s1[nOTs], r0[nOTs],
r1[nOTs], w, xtmp, ytmp;
    fe *g, *w0, *w1, *R0, *R1, *x, *y, *Y, *Z0, *Z1, *z0, *z1, *ztmp;//EC2 g,
w0, w1, R0, R1, x, y, Y, Z0, Z1, z0, z1, ztmp;
    brickexp *bg, *bx;//ebrick2 bg, bx;
...
EC2(*m_X, *m_Y);
    ...
    for(uint32_t k = 0; k < nOTs; k++)
    {
            ...
            //compute w0 and export it
            bx->pow(ztmp, s0[k]);//Miracl_mulbrick(&bx, s0[k].getbig(),
xtmp.getbig(), ytmp.getbig());
            //Miracl_InitPoint(&ztmp, xtmp, ytmp);//ztmp = EC2(xtmp, ytmp);
            bg->pow(R0, r0[k]);//Miracl_mulbrick(&bg, r0[k].getbig(),
xtmp.getbig(), ytmp.getbig());
            //Miracl_InitPoint(&R0, xtmp, ytmp);//R0 = EC2(xtmp, ytmp);
            w0->set(ztmp);//w0 = ztmp;
            w0->set_mul(w0, R0);//w0 += R0;
            w0->export_to_bytes(pBufIdx);//PointToByteArray(pBufIdx, coordSize,
w0);
            pBufIdx += febytelen;//coordSize + 1;

            //compute w1 and export it
            bx->pow(ztmp, s1[k]);//Miracl_mulbrick(&bx, s1[k].getbig(),
xtmp.getbig(), ytmp.getbig());
            //Miracl_InitPoint(&ztmp, xtmp, ytmp);//ztmp = EC2(xtmp, ytmp);
            bg->pow(R1, r1[k]);//Miracl_mulbrick(&bg, r1[k].getbig(),
xtmp.getbig(), ytmp.getbig());
            //Miracl_InitPoint(&R1, xtmp, ytmp);//R1 = EC2(xtmp, ytmp);
            w1->set(ztmp);//w1 = ztmp;
            w1->set_mul(w1, R1);//w1 += R1;

            w1->export_to_bytes(pBufIdx);//PointToByteArray(pBufIdx, coordSize,
w1);
            pBufIdx += febytelen;//coordSize + 1;
    }
    ...
    {
            //get y, z0, and z1
            y->import_from_bytes(pBufIdx);//ByteArrayToPoint(&y, coordSize,
pBufIdx);
            pBufIdx += febytelen;//coordSize+1;
            z0->import_from_bytes(pBufIdx);//ByteArrayToPoint(&z0, coordSize,
pBufIdx);
            pBufIdx += febytelen;//coordSize+1;
            z1->import_from_bytes(pBufIdx);//ByteArrayToPoint(&z1, coordSize,
pBufIdx);
            pBufIdx += febytelen;//coordSize+1;

            //compute first possible hash
            //cout << "r0: " << r0[k] << ", y: " << y << ", s0: " << s0[k] << ",
z0: " << z0 << ", ztmp: " << ztmp << endl;
            ztmp->set_double_pow_mul(y, r0[k], z0, s0[k]);
//ecurve2_mult2(r0[k].getbig(), y.get_point(), s0[k].getbig(), z0.get_point(),
ztmp.get_point());
//#endif
            //cout << "r0: " << r0[k] << ", y: " << y << ", s0: " << s0[k] << ",
z0: " << z0 << ", ztmp: " << ztmp << endl;
            //export result and hash
            ztmp->export_to_bytes(pBuf);//PointToByteArray(pBuf, coordSize,
ztmp);
            hashReturn(retPtr, hashbytelen, pBuf, febytelen, k);
            retPtr += hashbytelen;

            //compute second possible hash
            ztmp->set_double_pow_mul(y, r1[k], z1,
s1[k]);//ecurve2_mult2(r1[k].getbig(), y.get_point(), s1[k].getbig(),
z1.get_point(), ztmp.get_point());
            //export result and hash
            ztmp->export_to_bytes(pBuf);//PointToByteArray(pBuf, coordSize,
ztmp);
            hashReturn(retPtr, hashbytelen, pBuf, febytelen, k);
