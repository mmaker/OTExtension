...
class NNOBOTExtSnd : public OTExtSnd {
  public:
    NNOBOTExtSnd(uint32_t nSndVals, crypto *crypt, RcvThread *rcvthread,
                 SndThread *sndthread, bool dobaseots = true) {
  ...
  private:
    nnob_snd_check_t *UpdateCheckBuf(uint8_t *tocheckseed, uint8_t *tocheckrcv,
    ...
    BOOL CheckConsistency(queue<nnob_snd_check_t *> *check_buf_q,
                          channel *check_chan);
