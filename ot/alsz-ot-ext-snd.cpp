...
BOOL ALSZOTExtSnd::sender_routine(uint32_t id, uint64_t myNumOTs) {
    ...
    channel *check_chan =
        new channel(nchans * id + 1, m_cRcvThread, m_cSndThread);
    channel *mat_chan;
    if (use_mat_chan) {
    ...
    {
        processedOTBlocks =
            min((uint64_t)NUMOTBLOCKS, ceil_divide(lim - OT_ptr, wd_size_bits));
...
#endif
        rcvbufptr =
            ot_chan->blocking_receive_id_len(&rcvbuftmpptr, &tmpctr, &tmpotlen);
        // vRcv.AttachBuf(rcvbuftmpptr, bits_in_bytes(m_nBaseOTs *
        // OTsPerIteration));
        vRcv.SetBytes(rcvbuftmpptr, bits_in_bytes(OTsPerIteration * startpos),
                      bits_in_bytes((m_nBaseOTs - startpos) * OTsPerIteration));
        free(rcvbufptr);
// vRcv.PrintHex();
#ifdef OTTiming
        ...
        gettimeofday(&tempStart, NULL);
#endif
        check_queue.push(UpdateCheckBuf(Q.GetArr(), vRcv.GetArr(), OT_ptr,
                                        processedOTBlocks, check_chan));
        ...
        gettimeofday(&tempStart, NULL);
#endif
        HashValues(Q, seedbuf, vSnd, OT_ptr, min(lim - OT_ptr, OTsPerIteration),
                   rndmat);
#ifdef OTTiming
        gettimeofday(&tempEnd, NULL);
        ...
        if (check_chan->data_available()) {
            ...
            mask_queue.pop();
            MaskAndSend(tmpmaskbuf.maskbuf, tmpmaskbuf.otid, tmpmaskbuf.otlen,
                        ot_chan);
        }
#ifdef OTTiming
        gettimeofday(&tempEnd, NULL);
...
#ifdef OTTiming
    cout << "Sender time benchmark for performing " << myNumOTs << " OTs on "
         << m_nBitLength << " bit strings" << endl;
...
void ALSZOTExtSnd::FillAndSendRandomMatrix(uint64_t **rndmat,
                                           channel *mat_chan) {
    if (m_eSndOTFlav == Snd_GC_OT) {
...
alsz_snd_check_t ALSZOTExtSnd::UpdateCheckBuf(uint8_t *tocheckseed,
                                              uint8_t *tocheckrcv,
                                              uint64_t otid, uint64_t numblocks,
                                              channel *check_chan) {
    ...
    uint8_t *seedcheckbufptr = check_buf.seed_chk_buf,
            *rcvcheckbufptr = check_buf.rcv_chk_buf;
    ...
    for (uint64_t i = 0; i < m_nChecks;
         i++, seedcheckbufptr += OWF_BYTES, rcvcheckbufptr += OWF_BYTES) {
// memset(tmpbuf, 0, rowbytelen);
#ifdef DEBUG_ALSZ_CHECKS
        cout << i << "-th check between " << check_buf.perm[i].ida << " and "
             << check_buf.perm[i].idb << ": " << endl;
#endif
        ...
        for (uint64_t j = 0; j < iters; j++) {
            ((uint64_t *)tmpbuf[0])[j] =
                ((uint64_t *)pas)[j] ^ ((uint64_t *)pbs)[j];
            ((uint64_t *)tmpbuf[1])[j] = ((uint64_t *)tmpbuf[0])[j] ^
                                         ((uint64_t *)par)[j] ^
                                         ((uint64_t *)pbr)[j];
        }
        sha512_hash(seedcheckbufptr, OWF_BYTES, tmpbuf[0], rowbytelen,
                    hash_buf);
        ...
        XORandOWF(tocheckrcv + check_buf.perm[i].ida * rowbytelen, tocheckrcv +
        check_buf.perm[i].idb * rowbytelen,
...
void ALSZOTExtSnd::XORandOWF(uint8_t *idaptr, uint8_t *idbptr,
                             uint64_t rowbytelen, uint8_t *tmpbuf,
...
// m_cCrypt->hash_buf(resbuf, OWF_BYTES, tmpbuf, rowbytelen,
...
BOOL ALSZOTExtSnd::CheckConsistency(queue<alsz_snd_check_t> *check_buf_q,
                                    channel *check_chan) {
    uint8_t *rcvhashbufptr, *seedbufsrvptr, *rcvbufsrvptr, *rcvhashbuf;
    uint32_t ida, idb, receiver_hashes = 4;
    uint64_t checkbytelen = receiver_hashes * OWF_BYTES, tmpid, tmpnblocks,
             seedhashcli, rcvhashcli;

    uint8_t *rcvbuf =
        check_chan->blocking_receive_id_len(&rcvhashbuf, &tmpid, &tmpnblocks);
    uint8_t ca, cb;

    alsz_snd_check_t check_buf = check_buf_q->front();
    check_buf_q->pop();

    // Should be fine since the blocks are handled sequentially - but recheck
    // anyway
    assert(check_buf.otid == tmpid);
    assert(check_buf.numblocks == tmpnblocks);

    uint32_t blockoffset =
        ceil_divide(check_buf.otid, NUMOTBLOCKS * m_nBlockSizeBytes);
    uint32_t offset = 0; // m_nBaseOTs * blockoffset;//TODO, put offset in again
                         // when 3-stop ot is implemented
    ...
    for (uint32_t i = 0, j; i < m_nChecks; i++, rcvhashbufptr += checkbytelen) {
        ...
        for (j = 0; j < ceil_divide(OWF_BYTES, sizeof(uint64_t)); j++,
            seedbufsrvptr += sizeof(uint64_t),
            rcvbufsrvptr += sizeof(uint64_t)) {
            seedhashcli =
                *(((uint64_t *)rcvhashbufptr) + (2 * ca + cb) * 2 + j);
            rcvhashcli = *(((uint64_t *)rcvhashbufptr) +
                           (2 * (ca ^ 1) + (cb ^ 1)) * 2 + j);

            if (seedhashcli != *((uint64_t *)seedbufsrvptr) ||
                rcvhashcli != *((uint64_t *)rcvbufsrvptr)) {
#ifdef DEBUG_ALSZ_CHECKS
                cout << "Error in " << i << "-th consistency check between "
                     << ida << " and " << idb << " : " << endl;
                cout << "Receiver seed = " << (hex)
                     << ((uint64_t *)(rcvhashbufptr +
                                      ((2 * ca + cb) * OWF_BYTES)))[0]
                     << ((uint64_t *)(rcvhashbufptr +
                                      ((2 * ca + cb) * OWF_BYTES) + j))[1]
                     << ", my seed: " << ((uint64_t *)seedbufsrvptr)[0]
                     << ((uint64_t *)seedbufsrvptr)[1] << (dec) << endl;
                cout << "Receiver sndval = " << (hex)
                     << ((uint64_t *)(rcvhashbufptr +
                                      ((2 * (ca ^ 1) + (cb ^ 1)) * OWF_BYTES) +
                                      j))[0]
                     << ((uint64_t *)(rcvhashbufptr +
                                      ((2 * (ca ^ 1) + (cb ^ 1)) * OWF_BYTES) +
                                      j))[1]
                     << ", my snd val = " << ((uint64_t *)rcvbufsrvptr)[0]
                     << ((uint64_t *)rcvbufsrvptr)[1] << (dec) << endl;
#endif
                return false;
            }
        }
...
void ALSZOTExtSnd::genRandomPermutation(linking_t *outperm, uint32_t nids,
    ...
    for (uint32_t i = 0, rndctr = 0, j; i < nperms; i++) {
        ...
        // cout << "Permutation " << i << ": " << outperm[i].ida << " <-> " <<
        // outperm[i].idb << endl;
    }
