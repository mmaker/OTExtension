...
class OTExtRec : public OTExt {
     ...
     *byte size of: nOTs * state.field_size
     *
     * Output: was the execution successful?
     */
  public:
    OTExtRec(){};
    BOOL receive(uint64_t numOTs, uint64_t bitlength, CBitVector *choices,
                 CBitVector *ret, snd_ot_flavor stype, rec_ot_flavor rtype,
  ...
  protected:
    ...
    void InitRec(uint32_t nSndVals, crypto *crypt, RcvThread *rcvthread,
    ...
    void BuildMatrices(CBitVector &T, CBitVector &SndBuf, uint64_t ctr,
                       uint64_t numblocks);
    void MaskBaseOTs(CBitVector &T, CBitVector &SndBuf, uint64_t OTid,
                     uint64_t numblocks);
    void SendMasks(CBitVector Sndbuf, channel *chan, uint64_t OTid,
                   uint64_t processedOTs);
    void HashValues(CBitVector *T, CBitVector *seedbuf, CBitVector *maskbuf,
                    uint64_t ctr, uint64_t lim, uint64_t **mat);
    void SetOutput(CBitVector *maskbuf, uint64_t otid, uint64_t otlen,
                   queue<mask_block *> *mask_queue, channel *chan);
    void ReceiveAndUnMask(channel *chan, queue<mask_block *> *mask_queue);
    void ReceiveAndXORCorRobVector(CBitVector &T, uint64_t OT_len,
                                   channel *chan);
