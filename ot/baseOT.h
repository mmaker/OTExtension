...
class BaseOT {
  public:
    ...
    virtual void Sender(uint32_t nSndVals, uint32_t nOTs, channel *chan,
                        uint8_t *ret) = 0;
    virtual void Receiver(uint32_t nSndVals, uint32_t uint32_t,
                          CBitVector &choices, channel *chan, uint8_t *ret) = 0;

  protected:
    crypto *m_cCrypto;
    pk_crypto *m_cPKCrypto;

    void hashReturn(uint8_t *ret, uint32_t ret_len, uint8_t *val,
                    uint32_t val_len, uint64_t ctr) {
