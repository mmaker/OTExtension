...
class ALSZOTExtSnd : public OTExtSnd {
  public:
    ALSZOTExtSnd(uint32_t nSndVals, crypto *crypt, RcvThread *rcvthread,
                 SndThread *sndthread, uint32_t nbaseots, uint32_t nchecks,
                 bool dobaseots = true) {
  ...
  private:
    alsz_snd_check_t UpdateCheckBuf(uint8_t *tocheckseed, uint8_t *tocheckrcv,
                                    uint64_t otid, uint64_t numblocks,
                                    channel *check_chan);
    void XORandOWF(uint8_t *idaptr, uint8_t *idbptr, uint64_t rowbytelen,
                   uint8_t *tmpbuf, uint8_t *resbuf, uint8_t *hash_buf);
    void genRandomPermutation(linking_t *outperm, uint32_t nids,
                              uint32_t nperms);
    BOOL CheckConsistency(queue<alsz_snd_check_t> *check_buf_q,
                          channel *check_chan);
