...
void NNOBOTExtSnd::FillAndSendRandomMatrix(uint64_t **rndmat,
                                           channel *mat_chan) {
    if (m_eSndOTFlav == Snd_GC_OT) {
...
void NNOBOTExtSnd::XORandOWF(uint8_t *idaptr, uint8_t *idbptr,
...
#else
    m_cCrypt->hash_buf(
        resbuf, OWF_BYTES, tmpbuf, rowbytelen,
        hash_buf); // hash_buf, rowbytelen, tmpbuf, resbuf, hash_buf);
#endif
#ifdef DEBUG_NNOB_CHECKS_OUTPUT
    cout << "XOR-OWF Output:\t" << (hex);
...
BOOL NNOBOTExtSnd::CheckConsistency(queue<nnob_snd_check_t *> *check_buf_q,
                                    channel *check_chan) {
    ...
    for (uint32_t i = 0; i < m_nChecks; i++) {
        for (uint32_t j = 0; j < OWF_BYTES / sizeof(uint64_t);
             j++, rcvbufptr++, chkbufptr++) {
            if (*rcvbufptr != *chkbufptr) {
#ifdef DEBUG_NNOB_CHECKS
                cout << "Error in " << i << "-th consistency check: " << endl;
                cout << "Receiver hash = " << (hex) << *rcvbufptr
                     << ", my hash: " << *chkbufptr << endl;
#endif
                return FALSE;
            }
        }
